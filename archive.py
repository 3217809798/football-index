...
#   （如 2026-09-22 周二 → 只留「周二00X」这类），**没有星期序号的也删**。
#   竞彩必发源一次抓取会吐出多天赛事（周二那天的抓取里常混着周四、周五才踢的场次），
#   星期序号是「这场属哪一期竞彩」的权威标记 —— 由 nowscore 竞彩销售页提供，
#   比按开赛时间推算更准：凌晨场归前一销售期、不会被 noon-to-noon 窗口错放。
#   ⚠️ 无序号 ≠ 属于当天：2026-09-22 那 8 场无序号场次开赛时间都在两天后，
#      只是那一期竞彩还没开售、nowscore 暂时给不出序号，留着就是往归档里塞错日期的数据。
#
...
def keep_same_weekday(matches, day):
    """归档过滤：只留「星期序号 == 归档日星期」的比赛，其它一律删。

    ⚠️ 2026-09-23 修订：**没有星期序号的比赛也要删**（早期版本是保守保留）。
        线上 /football/2026-09-22/ 就栽在这里 —— 那天只该有「周二002/003/004」
        三场英锦赛，但源站一次吐出的 8 场欧国联 / 国际友谊 / 美职业都**没有星期序号**
        （它们 09-24、09-25 才开赛，属于更晚的竞彩销售期，nowscore 尚未给出序号），
        被「无序号保留」这条留了下来，于是当天归档里混进了两天后才踢的比赛。
        用户口径：归档日 D 只留带 D 星期序号的场次，其余删除。

    唯一护栏：**整批比赛一个星期序号都没有**时才退回不过滤。
        那说明 nowscore 补序号整体失败（enrich 挂了），此时按上面的规则会删成 0 场 ——
        一页空归档比「多留了几场」更糟，所以宁可原样保留，等下一轮补上序号再过滤。
    """
    tag = weekday_tag_of(day)
    if not tag:
        return list(matches or [])
    ms = list(matches or [])
    tagged = [m for m in ms if match_weekday_tag(m) is not None]
    if not tagged:                       # enrich 整体失败：保守，不删
        return ms
    return [m for m in ms if match_weekday_tag(m) == tag]
